...
public partial class Temp_TopPanel : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (common.myStr(Request.QueryString["IsEMRPopUp"]) == "1" || common.myStr(Request.QueryString["FromEMR"]) == "1")
            {
    ...
    protected void FillPatientData()
    {
        try
        {
            ...
            if (Request.QueryString["AlertType"] != null)
            {
                if (common.myInt(Session["EncounterId"]) != common.myInt(Request.QueryString["EId"]) && common.myInt(Request.QueryString["EId"]) != 0 && common.myInt(Request.QueryString["WardDetails"]).Equals(1))
                {
                    dsPatientDetail = (DataSet)Session["TopPanelPatientDetail" + common.myInt(Session["UserId"])];
                }
            }
            ...
            if (dsPatientDetail != null)
            {
                ...
                if (dsPatientDetail.Tables.Count > 0)
                {
                    if (dsPatientDetail.Tables[0].Rows.Count > 0)
                    {
                        ...
                        //}
                        lblCId.Text = common.myStr(dsPatientDetail.Tables[0].Rows[0]["RegistrationNo"]);
                        lblDob.Text = common.myStr(dsPatientDetail.Tables[0].Rows[0]["DOB"]);
                        //lblAge.Text = dsPatientDetail.Tables[0].Rows[0]["Age"].ToString();
                        //lblGender.Text = dsPatientDetail.Tables[0].Rows[0]["Gender"].ToString();
                        lblAge.Text = common.myStr(dsPatientDetail.Tables[0].Rows[0]["AgeGender"]);

                        if (common.myLen(dsPatientDetail.Tables[0].Rows[0]["BloodGroup"]) > 0)
                            lblAge.Text = lblAge.Text + "/" + common.myStr(dsPatientDetail.Tables[0].Rows[0]["BloodGroup"]);
                        ...
                        ViewState["Wardid"] = dsPatientDetail.Tables[0].Rows[0]["WardId"].ToString();

                        //Session["PatientWardId"] = dsPatientDetail.Tables[0].Rows[0]["WardId"].ToString();
                        //Session["PatientBedNo"] = dsPatientDetail.Tables[0].Rows[0]["BedNo"].ToString();

                        //lblEncNo.Text = lblEncNo_Resources.Text +": " + dsPatientDetail.Tables[0].Rows[0]["EncounterNo"].ToString();
                        lblEncNo.Text = dsPatientDetail.Tables[0].Rows[0]["EncounterNo"].ToString();
                    ...
                    if (dsPatientDetail.Tables[0].Rows.Count > 0)
                    {
                        if (dsPatientDetail.Tables[0].Rows[0]["PatientImage"].ToString() != "")
                        {
                            try
                            {
                                ...
                                //PatientImage.ImageUrl = "~/PatientDocuments/PatientImages/" + FileName;
                                // ViewState["PatientImages"] = "/PatientDocuments/PatientImages/" + FileName;
                                PatientImage.ImageUrl = "~/PatientDocuments/PatientImages/" + UnquieID + ".jpg";
