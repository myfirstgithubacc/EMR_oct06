...
public partial class Include_Components_MasterComponent_ucCaseSheet : System.Web.UI.UserControl
{
   ...
   string sectionId, string EntryType, int RecordId, string GroupingDate = "", bool IsConfidential = false)
            ...
            if (objDs != null)
            {
                ...
                if ((sDisplayEnteredBy == "Y") || (sDisplayEnteredBy == "N" && common.myStr(Session["OPIP"]) == "I"))
                {
                    if ((objStr.ToString() != "" || IsConfidential == true) && bool.Parse(TabularType) == false)
                    {
                        ...
                        if (dvValues.ToTable().Rows.Count > 0)
                        {
                            objStr.Append("<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-family: Tahoma; font-size:8pt;'>Entered By: " + common.myStr(dvValues.ToTable().Rows[0]["EnterBy"]) + " on " + common.myStr(dvValues.ToTable().Rows[0]["VisitDateTime"]) + "</span>");
                        }
                        dvValues.Dispose();
                    }
                    else
                    {
                        if ((objStr.ToString() != "" || IsConfidential == true) && bool.Parse(TabularType) == true)
                        {
                            ...
                            if (dvValues.ToTable().Rows.Count > 0)
                            {
                                objStr.Append("<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style='font-family: Tahoma; font-size:8pt;'>Entered By: " + common.myStr(dvValues.ToTable().Rows[0]["EnterBy"]) + " on " + common.myStr(dvValues.ToTable().Rows[0]["EntryDate"]) + "</span>");
                            }
                            dvValues.Dispose();
                        }
                    }
        ...
        string VisitType, string DoctorName, DateTime VisitDate, int GrouId)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    CallRecursive(tvName, iNodeID, "C" + iParentID.ToString() + "|" + TemplateId.ToString() + "|" + VisitType.ToString() + "|" + DoctorName.ToString() + "|" + VisitDate.ToString() + "|" + GrouId.ToString(), sNodeText);
                }
                tvName.CollapseAll();
            }
        }
    ...
    private void BindProvider()
    {
        ...
        try
        {
            ds = emr.fillDoctorCombo(Convert.ToInt16(Session["HospitalLocationId"]), 0, Convert.ToInt16(Session["FacilityId"]));
