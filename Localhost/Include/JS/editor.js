...
this.btnList = {
    ...
    "about":          ['about',                'Insert Site Link',  'editor_about(\''+objname+'\')',  'ed_about.gif'],

    // Add custom buttons here:
    "custom1":           ['custom1',              'Print Document',  'editor_action(this.id)',  'print.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  if (!config.height || config.height == "auto") {
    ...
    else if (obj.rows)         { config.height = obj.rows * 20 }       // row height
    else                       { config.height = '400'; }              // default
  }
...
function editor_action(button_id) {
  ...
  // Custom1
    ...
    editdoc.execCommand('print',0,val);
  }
...
//      TODO: Add our own undo/redo functionality
  ...
  if(ctrlKey && (ord==86))
    {
        var rVal = editor_obj.contentWindow.document.execCommand("Paste");
        editor_obj.contentWindow.document.body.focus()
        var stext = CleanHtml(editor_obj.contentWindow.document.body.innerHTML);
		if(stext != undefined)
		{
			editor_obj.contentWindow.document.body.innerHTML = "";
			var x = editor_obj.contentWindow.document.selection.createRange();
			x.pasteHTML(stext);
		}
		editEvent.cancelBubble = true ;
		editEvent.returnValue=false;
...
//Clean Html 
function CleanHtml(szHtml)
{
	var sText;	
	if(szHtml != '')
	{	
		sText = szHtml;
		sText = sText.replace(new RegExp('>&nbsp;<','gi'),'><');
		sText = sText.replace(new RegExp('\s*class=[A-Za-z0-9"]*','gi'),'');		
		sText = sText.replace(new RegExp('\s*style="?[^">]*"?','gi'),'');
		sText = sText.replace(new RegExp('<\/?font[^>]*>','gi'),'');
		sText = sText.replace(new RegExp('<\/?o:p[^>]*>','gi'),'');
		sText = sText.replace(new RegExp('<\/?st1[^>]*>','gi'),'');
		sText = sText.replace(new RegExp('<\/?span[^>]*>','gi'),'');
		sText = sText.replace(new RegExp('<h[1-5]{1}','gi'),'<B');
		sText = sText.replace(new RegExp('<\/h[1-5]{1}','gi'),'</B');		
		sText = sText.replace(new RegExp('<P *> *<BR *>','gi'),'<P>');
		sText = sText.replace(new RegExp('<P *> *<BR *>','gi'),'<P>');
		sText = sText.replace(new RegExp('<BR *> *<P *>','gi'),'<P>');
		sText = sText.replace(new RegExp('<BR *> *</P>','gi'),'</P>');
		sText = sText.replace(new RegExp('<\/P><BR>','gi'),'</P>');
		sText = sText.replace(new RegExp('<P *><\/P>|<P><B><\/B></P>|<P><strong><\/strong><\/P>|<DIV><\/DIV>','gi'),'');		
		while(sText.indexOf(document.location.href)>=0)
		{
		    sText = sText.replace(document.location.href,'');
		}
		sText = sText.replace(new RegExp('href="','gi'),'href="http://');
        sText = sText.replace(new RegExp('src="','gi'),'src="http://');        
		var objReg = new RegExp('&nbsp;&nbsp;');
		var objEx = objReg.exec(sText);
		while(objEx != null)
		{
		    sText = sText.replace(new RegExp('&nbsp;&nbsp;','gi'),'&nbsp;');
		    var objEx = objReg.exec(sText);
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  if (mode == "textedit" || editor_obj.tagName.toLowerCase() == 'iframe') {
    ...
    editor_obj.onpaste     = function() { editor_event(objname, 300); }
  ...
  else 
  {
    ...
    html += '</style>\n'
      + '</head>\n'
      + '<body contenteditable="true" style="padding:10px;" topmargin=10 leftmargin=10'
...
//      + ' oncontextmenu="parent.editor_cMenu_generate(window,\'' +objname+ '\');"'
    ...
    editdoc.body.onpaste   = function() { editor_event(objname, 300); }
...
function editor_insertHTML(objname, str1,str2, reqSel) {
 ...
 // insertHTML for wysiwyg iframe
  if (tagname == 'iframe') {
    ...
    if (sHtml.length && sHtml.length!='undefined') 
    {    
                              // if content selected           
     if (str2) 
        { 
