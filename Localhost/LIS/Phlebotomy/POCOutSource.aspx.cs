...
public partial class LIS_Phlebotomy_OutSource : System.Web.UI.Page
{
    ...
    protected void Page_PreInit(object sender, System.EventArgs e)
    {
        ...
        if (common.myStr(Request.QueryString["From"]).Equals("POPUP"))   // Sample Collection from Ward
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                if (common.myStr(Request.QueryString["callfrom"]) == "POC" && common.myInt(Request.QueryString["SEL_ServiceId"]) > 0)
                {
                    ...
                    int Age = age == "" ? 0 : common.myInt(age.Substring(0, 3).Trim());
                    ...
                    ViewState["IsEnableUserAuthentication"] = common.GetFlagValueHospitalSetup(common.myInt(Session["HospitalLocationId"]), common.myInt(Session["FacilityId"]),
                        "UserAuthenticationForLISAndPhlebotomy", sConString);
                    ...
                    if (common.myStr(Request.QueryString["SEL_StatusCode"]) == "DA")
                    {
                        btnsave.Visible = true;
                    }
                    else if (common.myStr(Request.QueryString["SEL_StatusCode"]) == "RE")
                    {
                        btnsave.Visible = true;
                    ...
                    if (!common.myStr(Request.QueryString["RegNo"]).Equals(""))
                    {
                        ...
                        BindTemplateGrid();
                        ViewState["StatusCode"] = common.myStr(Request.QueryString["SEL_StatusCode"]);
                    }
    ...
    protected void BindTemplateGrid()
    {
        ...
        try
        {
            lblServceName.Text = common.myStr(Request.QueryString["ServiceName"]);
            ...
            //{
            ViewState["PRESERVICEID"] = common.myInt(Request.QueryString["SEL_ServiceId"]);

            ViewState["PRERegistrationId"] = common.myInt(Request.QueryString["RegNo"]);
            ...
            string sSource = common.myStr(Session["OPIP"]).Length > 0 ? common.myStr(Session["OPIP"]) : "IPD";
            clsLISPhlebotomy oclsLISPhlebotomy = new clsLISPhlebotomy(sConString);
            sSource = sSource == "I" ? "IPD" : "OPD";
            string sStatus = common.myStr(ViewState["StatusCode"]);
            objDs = oclsLISPhlebotomy.getLabNoInvFormats(sSource, common.myInt(Session["FacilityId"]),
               common.myInt(Request.QueryString["SEL_DiagSampleID"]), common.myInt(Request.QueryString["SEL_ServiceId"]), gender,
               common.myInt(ViewState["Age"]), sStatus,
                common.myInt(Session["HospitalLocationID"]), 1);
            //Putting the Data in Global List
            collLabTemplate = new List<clsLabTemplate>();
            objLabTemplate = new clsLabTemplate();
            objLabTemplate.ServiceId = common.myInt(Request.QueryString["SEL_ServiceId"]);
            objLabTemplate.DiagSampleId = common.myInt(Request.QueryString["SEL_DiagSampleID"]);
            objLabTemplate.RegistrationId = common.myInt(Request.QueryString["RegNo"]);
            //objLabTemplate.StatusCode = common.myInt(Request.QueryString["RegNo"]);
            ...
            //else
            //{
            //    objDs = (DataSet)ViewState["collLabTemplate"];
            //}
    ...
    public void SaveData(string services)
    {
        ...
        holdPreviousData(common.myInt(Request.QueryString["SEL_ServiceId"]), common.myInt(Request.QueryString["SEL_DiagSampleID"]),
                        common.myInt(Request.QueryString["RegNo"]), common.myInt(ddlRemarks.SelectedValue), ReviewRemark,
                        common.myInt(ViewState["SampleTypeId"]));
        ...
        if (common.myStr(xmlfielddt) != "")
        {
            ...
            clsLISPhlebotomy oclsLISPhlebotomy = new clsLISPhlebotomy(sConString);
            int iResultId = 1, iStationId = 0, iDiagSampleId = 0, iRemarksId = 0;
            iLabNo = common.myInt(Request.QueryString["LabNo"]);
            iDiagSampleId = common.myInt(Request.QueryString["SEL_DiagSampleID"]);
            int SEL_ServiceId = common.myInt(Request.QueryString["SEL_ServiceId"]);
            iLabNo = common.myInt(Request.QueryString["LabNo"]);
            string sReleaseStage = "RF";

            strMsg = oclsLISPhlebotomy.saveInvResult("IPD", iLabNo, iResultId, iStationId, iDiagSampleId,
   common.myStr(xmlfielddt), 0, 0, sReleaseStage, 0, iRemarksId, common.myInt(Session["UserId"]), "", ReviewRemark,
   false, common.myInt(Session["FacilityId"]));
    ...
    protected void SaveFinalRelease()
    {
        ...
        if (common.myInt(Request.QueryString["SEL_DiagSampleID"]) != 0)
        {
            coll.Add(common.myInt(Request.QueryString["SEL_DiagSampleID"]));
            coll.Add("IPD");
            strXML.Append(common.setXmlTable(ref coll));
        }
        ...
        clsLISPhlebotomy objval = new clsLISPhlebotomy(sConString);
        string strMsg = objval.SaveResultFinalization("IPD", strXML.ToString(),
                              iEncodedUserId, common.myStr("F"), EmployeeId, common.myInt(Request.QueryString["StationId"]),
                              iCenterDoctor, iRightDoctor, iLeftDoctor, 0, 0, common.myInt(Session["FacilityId"]));
    ...
    protected void ddlMultilineFormats_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            holdPreviousData(common.myInt(Request.QueryString["SEL_ServiceId"]), common.myInt(Request.QueryString["SEL_DiagSampleID"]), common.myInt(Request.QueryString["RegNo"]), common.myInt(0), "0", common.myInt(0));
            ...
            if (collLabTemplate != null)
            {
                foreach (clsLabTemplate LT in collLabTemplate)
                {
                    if (LT.ServiceId == common.myInt(Request.QueryString["SEL_ServiceId"]) && LT.DiagSampleId == common.myInt(Request.QueryString["SEL_DiagSampleID"]))
                    {
    ...
    protected void ddlTemplateFieldFormats_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (collLabTemplate != null)
            {
                foreach (clsLabTemplate LT in collLabTemplate)
                {
                    if (LT.ServiceId == common.myInt(Request.QueryString["SEL_ServiceId"]) &&
                        LT.DiagSampleId == common.myInt(Request.QueryString["SEL_DiagSampleID"]))
                    {
                        DataRow[] dr = LT.dsField.Tables[OPTIONS].Select("FieldId=" + fieldId + "and ValueId=" + ddl.SelectedValue + "and ValueType= 'F'");
                        if (dr.Length > 0)
                        {
    ...
    protected void btnViewResult_Click(object sender, EventArgs e)
    {
        try
        {
            RadWindowForNew.NavigateUrl = "~/LIS/Phlebotomy/ViewOutsourcePatientHistory.aspx?RegID=" + common.myInt(Request.QueryString["RegNo"]) + "&EncounterId=" + common.myInt(Session["encounterid"]) + "";
