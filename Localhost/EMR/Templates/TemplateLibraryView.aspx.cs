...
public partial class EMR_Templates_TemplateLibraryView : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["TemplateLibraryName"] != null)
            {
                if (common.myStr(Session["TemplateLibraryName"]) != string.Empty)
                {
                    lblTemplateLibraryName.Text = common.myStr(Session["TemplateLibraryName"]);
                }
        ...
        //if (Request.QueryString["CopyTemplate"] != null)
        //{
        //    if (common.myStr(Request.QueryString["CopyTemplate"]) != string.Empty)
        //    {
        //        if (common.myStr(Request.QueryString["CopyTemplate"]) == "1")
        //        {
        asplHeaderUD.Visible = false;
        btnSave.Visible = false;
        btnPrintReport2.Visible = false;
        btnNew.Visible = false;
        lblTemplate2.Visible = false;
        ddlTemplateMain.Visible = false;
        ddlTemplatePatient.Visible = false;
        btnResultSetOpen.Visible = false;
    ...
    private void BindPermissionConfidentialUsers()
    {
        ...
        try
        {
            if (ddlTemplateMain.SelectedItem != null && common.myBool(ddlTemplateMain.SelectedItem.Attributes["IsConfidential"]) == true)
            {
    ...
    private void BindCategoryTree()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                hstInput.Add("@intTemplateId", common.myInt(Request.QueryString["TemplateId"]));
            ...
            if (Request.QueryString["TemplateId"] != null)
            {
                if (common.myStr(Request.QueryString["TemplateId"]) != string.Empty)
                {
                    ...
                    else
                    {
                        ...
                        TemplateId = common.myStr(Request.QueryString["TemplateId"]);
                    ...
                    if (!bTabular)
                    {
                        ...
                        hstInputStatic.Add("@intTemplateId", Request.QueryString["TemplateId"]);
                    ...
                    else
                    {
                        ...
                        if (objDsTabular.Tables[0].Rows.Count > 0)
                        {
                            ...
                            dsSectionRows = objDl.FillDataSet(CommandType.Text, "SELECT ValueId, ValueName FROM EMRTemplateRows WHERE SectionId = @intSectionId AND Active = 1 ORDER BY SequenceNo", hsSR);
    ...
    public DataTable BindFieldFormats(String strFieldId)
    {
        ...
        try
        {
            ...
            ds = objDl.FillDataSet(CommandType.Text, "Select FormatId, FormatName from EMRTemplateFieldFormats Where FieldId=@intFieldId And Active=1 Order by FormatName", hs);
                ...
                if (dvDataFilter.ToTable().Rows.Count > 0)
                {
                    #region Template Wise
                    {
                        ...
                        for (int i = 0; i < dtTemplate.Rows.Count; i++)
                        {
                            ...
                            //                    "1900-01-01",
                            //                    "1959-01-01", common.myStr(ViewState["OPIP"]), "");
                            //    if (sbTemp.ToString() != "")
                            //        TemplateString.Append(sbTemp + "<br/>");
                            //    sbTemp = null;
                            //    sbTemplateStyle = null;
    ...
    private string BindEditor(bool sign)
    {
        ...
        + "FROM EMRTemplateStatic where (HospitalLocationId = @inyHospitalLocationID or HospitalLocationId is null)";
    ...
    private void bindPatientTemplateList(bool IsNewLoad)
    {
        ...
        try
        {
            ...
            {
                item = new RadComboBoxItem();
                if (common.myBool(dr["IsConfidential"]) == true)
                {
    ...
    {
        if (common.myBool(ddlTemplateMain.SelectedItem.Attributes["IsConfidential"]) == true)
        {
        ...
        if (common.myInt(ddlTemplateMain.SelectedValue) > 0)
        {
            ...
            if (tvCategory.Nodes.Count > 0)
            {
                ...
                if (!common.myInt(ViewState["EncounterId"]).Equals(0)
                    ...
                    && !common.myInt(Session["FormID"]).Equals(0))
                {
                    string pullvalue = common.myStr(dl.ExecuteScalar(CommandType.Text, "select PullForward from EMRPatientForms epf INNER JOIN EMRPatientFormDetails epfd ON epf.PatientFormId = epfd.PatientFormId AND epfd.TemplateId = " + ViewState["PageId"] + " WHERE epf.EncounterId = " + common.myInt(ViewState["EncounterId"]) + " AND epf.RegistrationId =  " + common.myInt(ViewState["RegistrationId"]) + " AND epf.Active = 1 "));
            ...
            {
                if (common.myBool(ddlTemplatePatient.SelectedItem.Attributes["IsConfidential"]) == true)
                {
                    ViewState["ConfidentialTemplate"] = true;
                }
            }
            if (ddlTemplateMain.SelectedValue != "")
            {
                if (common.myBool(ViewState["DisplayIsConfidentialLink"]) == true)
                {
