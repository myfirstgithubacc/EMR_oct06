...
public partial class EMR_Order_TemplateRequestForm : System.Web.UI.Page
{
    ...
    protected void Page_PreInit(object sender, System.EventArgs e)
    {
        try
        {
            ...
            else if (common.myStr(Request.QueryString["IsEMRPopUp"]).Equals("1"))
            {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (!common.myStr(Request.QueryString["SourceForLIS"]).Trim().Equals(string.Empty))
            {
                ...
                //if (!common.myStr(Request.QueryString["Finalized"]).Trim().Equals(string.Empty))
                //{
                //    if (common.myStr(Request.QueryString["Finalized"]).Trim().Equals("RF"))
                //    {
                //        btnSave.Visible = false;
                //    }
                //    else
                ...
                && common.myStr(Session["EncounterStatus"]).ToUpper().Contains("CLOSE"))
            {
                ...
                && common.myStr(ViewState["SingleScreenTemplateCode"]).Equals("OTH")
                && common.myStr(Session["EncounterStatus"]).ToUpper().Contains("CLOSE"))
            {
            ...
            EnableSaving();
            if (lblManualLabNo.Text == "") { tblManualLabNo.Visible = false; }
            //btnSave.Visible = false;
        }
        try
        {
    ...
    private void SetPermission()
    {
        if (common.myInt(Session["ModuleId"]).Equals(3) || common.myInt(Session["ModuleId"]).Equals(30))
        {
            ...
            try
            {
                ...
                if (btnSave.Enabled || common.myBool(ViewState["IsChkFirstTemplateEditBy"]))
                {
                    clsIVF objI = new clsIVF(sConString);

                    int encodedBy = objI.getFirstTemplateEditBy(common.myInt(Session["EncounterId"]), common.myInt(ddlRecord.SelectedValue), common.myInt(ddlTemplateMain.SelectedValue));

                    if (encodedBy > 0)
                    {
                        if (!common.myInt(Session["UserID"]).Equals(common.myInt(encodedBy)))
                        {
                            btnSave.Enabled = false;
                            ViewState["IsChkFirstTemplateEditBy"] = true;
                        }
                        else
                        {
                            if (common.myBool(ViewState["IsChkFirstTemplateEditBy"]))
                            {
                                btnSave.Enabled = true;
                            }
                        }
    ...
    private void BindPermissionConfidentialUsers()
    {
        ...
        try
        {
            if (ddlTemplateMain.SelectedItem != null && common.myBool(ddlTemplateMain.SelectedItem.Attributes["IsConfidential"]))
            {
                ...
                if (dv.ToTable().Rows.Count > 0 || common.myInt(Session["EmployeeId"]).Equals(common.myInt(Session["DoctorId"])))
                {
                    ViewState["AllowConfidentialTemplate"] = true;
                }

                if (common.myInt(Session["EmployeeId"]).Equals(common.myInt(Session["DoctorId"])))
                {
                ...
                if (ds.Tables[1].Rows.Count > 0)
                {
                    ...
                    {
                        if (common.myInt(Session["EmployeeId"]).Equals(common.myInt(ds.Tables[1].Rows[i]["Encodedby"])))
                        {
                            ViewState["AllowConfidentialTemplate"] = true;
                        }
                    }
    ...
    private void addRowTabular()
    {
        try
        {
            ...
            DataRow[] drTT = dtNoOfColumn.Select("SectionId=" + common.myInt(ViewState["SelectedNode"]));
                ...
                else if (common.myStr(gvTabularFormat.Items[i].Cells[(byte)enumT.IsData].Text).Trim().Equals("D"))
                {
                    ...
                    for (int j = 0; j < common.myInt(ViewState["TotalColCount"]) - 6; j++) //change 5 to 6
                    {
                        ...
                        if (common.myStr(lblControlType.Text).Equals("T"))
                        {
                            ...
                            if (dtFieldMaxLength != null)
                            {
                                if (dtFieldMaxLength.Rows.Count > 0)
                                {
                                    ...
                                    DataView dvMaxLength = new DataView(dtFieldMaxLength);
                                    dvMaxLength.RowFilter = "FieldId=" + common.myInt(fieldid.Text);
                                    if (dvMaxLength.ToTable().Rows.Count>0)
                                    {
                                        txtVisData.Columns = (common.myInt(dvMaxLength.ToTable().Rows[0]["MaxLength"]) > 30) ? 30 : common.myInt(dvMaxLength.ToTable().Rows[0]["MaxLength"]);
                                        txtVisData.MaxLength = common.myInt(dvMaxLength.ToTable().Rows[0]["MaxLength"]);
                                    }
                                    dvMaxLength.Dispose();
                                }
                            }
                        ...
                        else if (common.myStr(lblControlType.Text).Equals("D"))
                        {
                            ...
                            ListItem l1 = new ListItem(string.Empty, "0");
                            ddlVisData.Items.Insert(0, l1);

                            ddlVisData.SelectedValue = common.myStr(lblControlType.Text);
                    ...
                    if (dr.Length > 0)
                    {
                        ...
                        if (ViewState["TabularMandatoryField"] != null)
                        {
                            ...
                            if (tblRefField != null)
                            {
                                ...
                                {
                                    DataView dvRefField = new DataView(tblRefField);
                                    dvRefField.RowFilter = "LEN(ISNULL(ReferenceName,''))>0 OR TotalCalc=1";
                                    if (dvRefField.ToTable().Rows.Count > 0)
                                    {
    ...
    private void BindCategoryTree()
    {
        ...
        try
        {
            ...
            DataSet ds = (DataSet)dl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetSectionTree", hstInput);
    ...
    protected void ddlTemplateFieldFormats_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            ds = dl.FillDataSet(CommandType.Text, "Select FormatText from EMRTemplateFieldFormats WITH (NOLOCK) Where FormatId = @intFormatId ", hs);
            if (ds.Tables[0].Rows.Count > 0)
            {
    ...
    protected void B_OnClick(object sender, EventArgs e)
    {
        try
        {
            ...
            foreach (GridViewRow r in gvSelectedServices.Rows)
            {
                ...
                {
                    if (common.myStr(row.Cells[(byte)enumNonT.FieldType].Text).Equals("B"))
                    {
    ...
    protected void BindBothGrids(bool bTabular)
    {
        DataSet ds;
        ArrayList ar;
        ...
        try
        {
            ...
            if (!bTabular)
            {
                ...
                hstInputStatic.Add("@intTemplateId", common.myInt(ddlTemplateMain.SelectedValue));
                ...
                objDs = (DataSet)objDl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetTemplateDetails", hstInputStatic);
            ...
            else
            {
                ...
                objDsTabular = (DataSet)objDl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetTemplateDetailsTabular", hstInputTabular);
                    ...
                    if (objDsTabular.Tables.Count > 4)
                    {
                        ...
                        if (ViewState["TabularMandatoryField"] != null)
                        {
                            ...
                            if (tblRefField != null)
                            {
                                if (tblRefField.Rows.Count > 0)
                                {
                                    tblRefField.DefaultView.RowFilter = "LEN(ISNULL(ReferenceName,''))>0 OR TotalCalc=1 ";
                                    ...
                                    //    dvFilter.RowFilter = "TotalCalc=1";
            ...
            if (!common.myInt(Session["HospitalLocationId"]).Equals(0) && !common.myInt(ViewState["EncounterId"]).Equals(0))
            {
                ...
                if (!common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Trim().Equals(string.Empty))
                {
                    ...
                    if (common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Equals("T"))
                    {
                        ...
                        if (!common.myBool(ViewState["AllowConfidentialTemplate"])
                            && common.myBool(ViewState["ConfidentialTemplate"]))
                        {
                            txtT.Enabled = false;
                            btnHelp.Enabled = false;
                    ...
                    if (common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Trim().Equals("I"))
                    {
                        ...
                        if (objDt != null)
                        {
                            ...
                            if (!common.myBool(ViewState["AllowConfidentialTemplate"]) && common.myBool(ViewState["ConfidentialTemplate"]))
                            {
                                txtT.Enabled = false;
                                btnHelp.Enabled = false;
                                Hy_LinkUrl.Enabled = false;
                    ...
                    {
                        cCtlType = "C";
                        Repeater rpt = (Repeater)e.Row.Cells[(byte)enumNonT.Values].FindControl("C");
                        ...
                        HiddenField hdnCV = (HiddenField)rpt.FindControl("hdnCV");
                        ...
                        {
                            btnAdd.Visible = true;
                            btnAdd.Attributes.Add("onclick", "openRadWindowForFieldValue('" + rpt.ClientID + "','C','" + common.myInt(e.Row.Cells[(byte)enumNonT.SectionId].Text) + "',' " + common.myInt(e.Row.Cells[(byte)enumNonT.FieldId].Text) + "','" + common.myStr(e.Row.RowIndex) + "')"); //"win=window.open('SentanceGallery.aspx?ctrlId=" + txtM.ClientID + "&typ=0','SentanceGallery','resizable=0,scrollbars=yes,width=600,height=520'); win.moveTo(40,150); return false;");
                        }
                    ...
                    {
                        cCtlType = "B";
                        //DropDownList ddlB = (DropDownList)e.Row.FindControl("B");
                    ...
                    #endregion
                    #region Date Type
                    else if (common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Equals("S") 
                        || common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Equals("ST") 
                        || common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text).Equals("SB"))//For Date
                    {
                        ...
                        if (objDt != null)
                        {
                            if (objDt.Rows.Count > 0)
                            {
                                ...
                                else if (FieldType.Equals("SB"))
                                {
                                    ...
                                    //ddlTime.Attributes.Add("onchange", "ctrlOnSelectedIndexChanged('" + ddlTime.ClientID + "','" + common.myInt(e.Row.Cells[(byte)enumNonT.FieldId].Text) + "','" + common.myStr(e.Row.Cells[(byte)enumNonT.FieldType].Text) + "')");
                                }
                            }
                        }
                        ...
                        if (IsMandatory)
                        {
                            ...
                            switch (FielType)
                            {
                                case "T":
                                    ...
                                    if (common.myStr(txtT.Text).Trim().Length.Equals(0) && txtT.Visible)
                                    {
                                        IsEntered = false;
                                    }
                                    break;
                                ...
                                case "M":
                                    ...
                                    if (common.myStr(txtM.Text).Trim().Length.Equals(0) && txtM.Visible)
                                    {
                                        IsEntered = false;
                                    }
                                    break;
                                ...
                                case "W":
                                    ...
                                    if (common.myStr(txtW.Content).Trim().Length.Equals(0) && txtW.Visible)
                                    {
                                        IsEntered = false;
                                    }
                                    break;
                                ...
                                case "C":
                                    Repeater rptC = (Repeater)item2.FindControl("C");
                                    bool isChk = false;
    ...
    private void saveData(bool isSaveManual)
    {
        ...
        try
        {
            if (!btnSave.Enabled && !isSaveManual)
            {
                return;
            }
                    ...
                    else if (common.myStr(gvTabularFormat.Items[i].Cells[(byte)enumT.IsData].Text).Trim().Equals("D"))
                    {
                        ...
                        for (int j = 0; j < common.myInt(ViewState["TotalColCount"]) - 6; j++)
                        {
                            ...
                            else if (lblControlType.Text.Equals("D"))
                            {
                                ...
                                DropDownList ddlVisData = (DropDownList)gvTabularFormat.Items[i].Cells[j].FindControl("D" + j.ToString());
                                if (common.myStr(ddlVisData.SelectedValue) != "")
                                {
                                        ...
                                        if (ViewState["TabularMandatoryField"] != null)
                                        {
                                            ...
                                            if (tbl != null)
                                            {
                                                if (tbl.Rows.Count > 0)
                                                {
                                                    ...
                                                    DV.RowFilter = "FieldId=" + common.myInt(FieldId);
                        ...
                        if (dsTemplateDataDetails.Tables.Count > 0)
                        {
                            ...
                            foreach (GridViewRow dataItem in gvTemplateRequiredServices.Rows)
                            {
                                    ...
                                    || !IsAnyChk)
                                {
                                        ...
                                        if (item2.RowType.Equals(DataControlRowType.DataRow))
                                        {
                                            ...
                                            else if (common.myStr(item2.Cells[(byte)enumNonT.FieldType].Text).Equals("C"))
                                            {
                                                ...
                                                foreach (RepeaterItem rptItem in rptC.Items)
                                                {
                                                    ...
                                                    sCheckedValues = chk.Checked ? hdn.Value : "0";
                                                    coll.Add(item2.Cells[(byte)enumNonT.FieldId].Text);
                                                    coll.Add("C");
                                    ...
                                    {
                                        ViewState["ResultSet"] = strNonTabular.ToString();
                                    }
                                    else
                                    {
                                        ViewState["ResultSet"] = common.myStr(ViewState["ResultSet"]) + strNonTabular.ToString();
                                    }
                    ...
                    foreach (GridViewRow item2 in gvSelectedServices.Rows)
                    {
                        if (item2.RowType.Equals(DataControlRowType.DataRow))
                        {
                            ...
                            else if (common.myStr(item2.Cells[(byte)enumNonT.FieldType].Text).Equals("D"))
                            {
                                ...
                                coll.Add(item2.Cells[(byte)enumNonT.FieldId].Text);
                                coll.Add("D");
                                coll.Add(common.myStr(ddl.SelectedValue));
                            ...
                            {
                                RadioButtonList ddlB = (RadioButtonList)item2.FindControl("B");
                                if (common.myStr(ddlB.SelectedValue) != "")
                                {
                                    if (!ddlB.SelectedItem.Text.Equals("Select"))
                                    {
                                        coll.Add(item2.Cells[(byte)enumNonT.FieldId].Text);
            ...
            if (!strNonTabular.ToString().Equals(string.Empty) || !strTabular.ToString().Equals(string.Empty))
            {
                ...
                string strMsg = objEMR.SavePatientNotesData(common.myInt(Session["HospitalLocationID"]), common.myInt(Session["FacilityID"]), 1,
                                            ...
                                            common.myInt(Session["UserID"]), OrderDetailId, OrderRequestId, SubDeptId, intProviderId, dtChangeDate,false,0);
                ...
                {
                    if ((common.myStr(Request.QueryString["From"]).Equals("POPUP") || common.myStr(Request.QueryString["IsEMRPopUp"]).Equals("1"))
                        && !common.myStr(Request.QueryString["SingleScreenTemplateCode"]).Equals(string.Empty)
                        && !common.myStr(Request.QueryString["SingleScreenTemplateCode"]).Equals("OTH"))
                    {
    ...
    private string PrintReport(bool sign)
    {
        ...
        sb.Append(getReportHeader(5).ToString());//ReportId

        clsIVF objIVF = new clsIVF(sConString);

        string strPatientHeader = objIVF.getCustomizedPatientReportHeader(1); //common.myInt(ddlReport.SelectedItem.Attributes["HeaderId"])
        ...
        try
        {
            string DoctorId = fun.GetDoctorId(common.myInt(Session["HospitalLocationID"]), common.myInt(common.myInt(Session["UserID"])));
            ...
            if (ddlTemplateMain.SelectedValue != "")
            {
                ...
                dsTemplateData = emr.GetDiagPrintTemplate(common.myInt(Session["HospitalLocationId"]),
                    common.myInt(Session["FacilityId"]), common.myInt(Session["RegistrationId"]), common.myInt(Session["EncounterId"]), "1900-01-01",
                    "2059-01-01", "", common.myInt(ddlTemplateMain.SelectedValue), "D", false, 0);
            }
            else
            {
                        ...
                        foreach (DataRow item in objDs.Tables[0].Rows)
                        {
                                ...
                                || common.myStr(item["FieldsListStyle"]) != "")
                            {
                                ...
                                if (objDt.Rows.Count > 0)
                                {
                                    ...
                                    if (Convert.ToBoolean(item["DisplayTitle"]))
                                    {
                                        ...
                                        objStr.Append(BeginList + sBegin + "<br/><b>" + common.myStr(item["FieldName"]) + " : </b>");
                                        //}
                                        //else
                                        //{
                            ...
                            else
                            {
                                if (objDt.Rows.Count > 0)
                                {
                                    ...
                                    {
                                        objStr.Append("<br/><b>" + common.myStr(item["FieldName"]) + "<b>");
                                    }
                                }
                            }
    ...
    private StringBuilder getReportHeader(int ReportId)
    {
        ...
        try
        {
            ...
            if (IsPrintHospitalHeader)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int idx = 0; idx < ds.Tables[0].Rows.Count; idx++)
                    {
                        ...
                        sb.Append("<td colspan='2' align ='right' valign='middle' style='font-size:9px'><img src='../../Icons/SmallLogo.jpg' border='0' width='150px' height='80px'  alt='Image'/></td>");
                        sb.Append("</tr>");
                        sb.Append("</table>");
                        sb.Append("</td>");
                        ...
                        sb.Append("<table border='0' cellpadding='0' cellspacing='0'>");
                        sb.Append("<tr>");
                        sb.Append("<td  align ='center'  style='font-size:9px'><b>" + common.myStr(DR["FacilityName"]).Trim() + "</b></td>");
                        sb.Append("</tr>");
                        sb.Append("<tr >");
            ...
            sb.Append("<td align=center><U></U></td>");
    ...
    private string BindEditor(bool sign)
    {
        ...
        if (dtTemplate.Rows.Count > 0)
        {
            foreach (DataRow dr in dtTemplate.Rows)
            {
                ...
                if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("VITALS"))
                {
                    int lck = 0;
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("LAB HISTORY"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (dr["PageName"].ToString().Trim().ToUpper().Equals("CHIEF COMPLAINTS"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("ICD DIAGNOSIS"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("ALLERGIES"))
                {
                    int lck = 0;
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("PRESCRIPTION"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("CURRENT MEDICATION"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("ORDERS AND PROCEDURES"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if (common.myStr(dr["PageName"]).Trim().ToUpper().Equals("IMMUNIZATION CHART"))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
                ...
                else if ((common.myStr(dr["PageName"]).Trim().ToUpper().Equals("ROS") || common.myStr(dr["TemplateType"]).Trim().ToUpper().Equals("ROS")))
                {
                    int lck = 0;
                    sbTemplateStyle = new StringBuilder();
                    DataView dv = new DataView(dsTemplateStyle.Tables[0]);
            ...
            else if (!sign)
            {
                if (Saved_RTF_Content != null)
                {
                    if (Saved_RTF_Content.Contains("dvDoctorImage"))
                    {
    ...
    public StringBuilder BindProblemsROS(int HospitalId, int EncounterId, StringBuilder sb, String sDisplayName, String sTemplateName, string pageID)
    {
        ...
        DataSet dsFont = (DataSet)DlObj.FillDataSet(CommandType.StoredProcedure, "UspEMRGetSectionTreeForPatientsForms", hstInput);
            ...
            for (int i = 0; i < dtPositiveRos.Rows.Count; i++)
            {
                ...
                if (common.myStr(dr["SectionId"]) != strSectionId)
                {
                    ...
                    if (common.myStr(drFont["SectionsBold"]) != "" || common.myStr(drFont["SectionsItalic"]) != "" || common.myStr(drFont["SectionsUnderline"]) != "" || common.myStr(drFont["SectionsFontSize"]) != "" || common.myStr(drFont["SectionsForecolor"]) != "" || common.myStr(drFont["SectionsListStyle"]) != "")
                    {
                        ...
                        if (common.myBool(drFont["SectionDisplayTitle"]))
                        {
                            if (i == 0)
                            {
                                objStrTmp.Append("<br />" + sBegin + "Positive Symptoms:" + sEnd);
    ...
    private string BindStaticTemplates(int StaticTemplateId, int TemplateFieldId)
    {
        ...
        string DoctorId = fun.GetDoctorId(HospitalId, common.myInt(UserId));
        ...
        if (dtTemplate.Rows.Count > 0)
        {
            if (common.myStr(dtTemplate.Rows[0]["TemplateName"]).Trim().ToUpper().Equals("ALLERGIES")
                && common.myStr(dtTemplate.Rows[0]["DataStatus"]).Trim().ToUpper().Equals("AVAILABLE"))
            {
            ...
            else if (common.myStr(dtTemplate.Rows[0]["TemplateName"]).Trim().ToUpper().Equals("VITALS")
                && common.myStr(dtTemplate.Rows[0]["DataStatus"]).Trim().ToUpper().Equals("AVAILABLE"))
            {
                string strTemplateType = common.myStr(dtTemplate.Rows[0]["PageIdentification"]);
                //strTemplateType = strTemplateType.Substring(0, 1);
            ...
            else if (common.myStr(dtTemplate.Rows[0]["TemplateName"]).Trim().ToUpper().Equals("DIAGNOSIS")
                && common.myStr(dtTemplate.Rows[0]["DataStatus"]).Trim().ToUpper().Equals("AVAILABLE"))
            {
                string strTemplateType = common.myStr(dtTemplate.Rows[0]["PageIdentification"]);
                //strTemplateType = strTemplateType.Substring(0, 1);
                ...
                bnotes.BindAssessments(RegId, HospitalId, EncounterId, common.myInt(UserId),
    ...
    protected void AddStr2(string type, ref string Saved_RTF_Content, StringBuilder sbTemp, string Lock, string Linespace, string ShowNote)
    {
        ...
        else
        {
            ...
            //{
            if (common.myBool(ShowNote) && (ViewState["PullForward"] == null || ViewState["DefaultTemplate"] == null))
            {
            ...
            {
                if (common.myBool(ViewState["DefaultTemplate"]))
                {
                    Replace(type, ref Saved_RTF_Content, sbTemp.ToString(), Lock);
                }
            }
            ...
            {
                if (common.myBool(ViewState["PullForward"]))
                {
                    Replace(type, ref Saved_RTF_Content, sbTemp.ToString(), Lock);
                }
            }
    ...
    protected void bindData(string iFormId, string TemplateId, string SectionId, StringBuilder sb)
    {
        ...
        DataSet ds = (DataSet)dl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetSectionTreeForPatientsForms", hstInput);
        ...
        if (common.myInt(ddlReport.SelectedValue) != 0 && SectionId != "" && ds.Tables[0].Rows.Count > 0)
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                dsGetData = (DataSet)dl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetTemplateDetails", hstInput);
        ...
        else
        {
            ...
            dsAllSectionDetails = (DataSet)dl.FillDataSet(CommandType.StoredProcedure, "UspEMRGetTemplateDetails", hstInput);
        }
    ...
    protected DataSet GetPageProperty(string iFormId)
    {
        ...
        if (!common.myInt(Session["HospitalLocationID"]).Equals(0) && iFormId != "")
        {
            if (Cache[common.myInt(Session["HospitalLocationID"]).ToString() + "_" + iFormId + "_FormPageSettings"] == null)
            {
                ...
                hstInput.Add("@inyHospitalLocationId", common.myInt(Session["HospitalLocationID"]));
                hstInput.Add("@intFormId", 1);
                DataSet ds = null;//(DataSet)dl.FillDataSet(CommandType.StoredProcedure, "EMRGetFormPageSettingDetails", hstInput);
            ...
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count.Equals(0))
                    {
                        ...
                        DR["RecordId"] = 1;
                        DR["RecordName"] = "1 - " + DateTime.Now.ToString("dd/MM/yyyy  hh:mmtt");
                    ...
                    if (IsAddVisit)
                    {
                        ...
                        if (rCount > 0)
                        {
                            int recordid = common.myInt(ds.Tables[0].Compute("MAX(RecordId)", string.Empty)) + 1;
                            DataRow DR = ds.Tables[0].NewRow();
                            DR["RecordId"] = recordid;
                            DR["RecordName"] = recordid.ToString() + " - " + DateTime.Now.ToString("dd/MM/yyyy  h:mmtt");
                            ds.Tables[0].Rows.InsertAt(DR, 0);
                        }
                    }
        ...
        foreach (GridViewRow row in gvSelectedServices.Rows)
        {
            ...
            if (common.myInt(hdnSelCell.Text).Equals(common.myInt(row.RowIndex)))
            {
                ...
                else if (FieldType.Equals("C"))
                {
                    ...
                    Repeater rpt = (Repeater)row.FindControl("C");
    ...
    private void bindMainTemplateList()
    {
        ...
        try
        {
            ...
            else if (common.myStr(ViewState["SingleScreenTemplateCode"]) != "" && common.myStr(ViewState["SingleScreenTemplateCode"]) == "OTH")
            {
                dvFilterTemplateCode.RowFilter = "TemplateTypeCode<>'EXM' AND TemplateTypeCode<>'HIS' AND TemplateTypeCode<>'POC' AND TemplateTypeCode<>'CA' AND TemplateTypeCode<>'PT' AND TemplateTypeCode<>'NS'";
            }
    ...
    private void BindTemplateRequiredService(string ServiceIds)
    {
        ...
        try
        {
                ...
                && common.myStr(ViewState["TagType"]).Equals("D"))
            {
                ...
                ds = (DataSet)objDl.FillDataSet(CommandType.StoredProcedure, "uspTemplateRequiredServices", hstInput);
    ...
    public DataSet getEMRTemplateForSelectedService(int HospId, string ServiceIds, int TemplateId)
    {
        ...
        try
        {
            ...
            //ds = (DataSet)objDl.FillDataSet(CommandType.Text, qry, HshIn);

            ds = (DataSet)objDl.FillDataSet(CommandType.StoredProcedure, "uspEMRGetServiceTemplates", HshIn);
        }
        catch (Exception Ex)
        {
            throw Ex;
                                    ...
                                    if (common.myStr(lblControlType.Text).Equals("T"))
                                    {
                                        if (dtFieldMaxLength != null)
                                        {
                                            if (dtFieldMaxLength.Rows.Count > 0)
                                            {
                                                ...
                                                DataView dvFieldMaxLength = new DataView(dtFieldMaxLength);
                                                dvFieldMaxLength.RowFilter = "FieldId=" + common.myInt(FieldId);
                                                if (dvFieldMaxLength.ToTable().Rows.Count > 0)
                                                {
                                                    int maxVal = getPixelValue(common.myInt(dvFieldMaxLength.ToTable().Rows[0]["MaxLength"]));

                                                    gvTabularFormat.Columns[j].ItemStyle.Width = Unit.Pixel(maxVal);
                                                    gvTabularFormat.Columns[j].HeaderStyle.Width = Unit.Pixel(maxVal);
                                                }

                                                dvFieldMaxLength.Dispose();
                                            }
                                        }
                                    }
                                    else if (common.myStr(lblControlType.Text).Equals("M"))
                                    ...
                                    else if (common.myStr(gvTabularFormat.Items[i].Cells[(byte)enumT.IsData].Text).Trim().Equals("D"))
                                    {
                                        ...
                                        for (int j = 0; j < common.myInt(ViewState["TotalColCount"]) - 6; j++)
                                        {
                                            ...
                                            if (common.myStr(lblControlType.Text).Equals("T"))
                                            {
                                                ...
                                                tblRefField.DefaultView.RowFilter = "FieldId=" + common.myInt(FieldId);
            ...
            if (ViewState["TabularMandatoryField"] != null)
            {
                ...
                if (tblRefField != null)
                {
                    if (tblRefField.Rows.Count > 0)
                    {
                        ...
                        if (isLastRow)
                        {
                            DV.RowFilter = "FieldId=" + FieldId + " AND TotalCalc=1";
                            if (DV.Count > 0)
                            {
    ...
    protected void ddlTemplatePatient_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            //ddlTemplateMain.SelectedValue = common.myStr(ddlTemplatePatient.SelectedValue);
    ...
    //        DAL.DAL objDl = new DAL.DAL(DAL.DAL.DBType.SqlServer, sConString);
    //        DataSet objDs = (DataSet)objDl.FillDataSet(CommandType.Text, sQ, hstInput);
