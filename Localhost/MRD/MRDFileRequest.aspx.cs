...
public partial class MRD_MRDFileRequest : System.Web.UI.Page
{
    ...
    private bool isSaved()
    {
        ...
        if (common.myInt(ddlDoctorList.SelectedValue) == 0)
        {
        ...
        {
            strmsg.Append("Please select Department !");
            isSave = false;
        }
        if (common.myStr(txtRemarks.Text).Length == 0)
        {
            strmsg.Append("Purpose/Remarks can't be blank !");
            isSave = false;
        }
    ...
    private void SaveData()
    {
        ...
        //try
        //{
        //    BaseC.Hospital HIS = new BaseC.Hospital(sConString);           
        //    DataSet ds = HIS.GetSpecialRightsEmployeeList(common.myInt(Session["HospitalLocationID"]), 0, common.myInt(Session["FacilityID"]), "IsUserHaveFileApprovalPermission");

        //    if(ds.Tables[0].Rows.Count<=0)
        //    {
        //        Alert.ShowAjaxMsg("User not tagged!", Page);
        //        return;
        //    } else
        //    {
        //        DataView DV = ds.Tables[0].DefaultView;
        //        DV.RowFilter = "EmployeeId=" + common.myInt(Session["UserID"]);
        //        if (DV.ToTable().Rows.Count <= 0)
        //        {
        //            Alert.ShowAjaxMsg("You are not authorized to issue file on request!", Page);
        //            return;
        //        }
        //    }
        ...
        try
        {
            ...
            {
                sFileName = CreateFileName("MRD", _FileUpload.FileName);
            ...
            else if (common.myStr(ViewState["PageType"]).ToUpper().Equals("ISS"))
            {
            ...
            {
                strMsg = objMRD.UpdateManualMRDFileIssueRetun(common.myInt(hdnGIssueId.Value),
            ...
            {
                strMsg = objMRD.MRDSaveRequestFile(common.myInt(Session["HospitalLocationID"]), common.myInt(Session["FacilityId"]),
                           common.myInt(hdnRegistrationId.Value), common.myInt(hdnEncounterId.Value),
                           common.myInt(ddlDoctorList.SelectedValue), common.myInt(ddlDepartment.SelectedValue),
                           dtpRequiredDate.SelectedDate.Value, common.myStr(txtRemarks.Text),
                           common.myStr(ViewState["PageType"]),
                           common.myBool(hdnIsPasswordRequired.Value) ? common.myInt(PasswordRequiredHelper.GetTransactionUserId()) : common.myInt(Session["UserId"]), // To swapping between current userid & transaction userid
                           sConString, common.myInt(ddlRequestBy.SelectedValue), ViewState["DocumentName"].ToString(), txtFileName.Text);
    ...
    void BindPatientHiddenDetails(int RegistrationNo)
    {
        try
        {
            ...
            if (RegistrationNo > 0)
            {
                ...
                ds1 = objMRD.GetMRDReturnFile(0, common.myInt(hdnRegistrationId.Value), common.myInt(Session["HospitalLocationID"]), common.myInt(Session["FacilityID"]), "I", Convert.ToDateTime("1900/01/01"), Convert.ToDateTime("2079/01/01"), sConString, common.myStr(RegistrationNo), common.myStr(hdnEncounterNo.Value), "");

                gvData1.DataSource = ds1.Tables[0];
                gvData1.DataBind();
            }
        }
        catch (Exception ex)
